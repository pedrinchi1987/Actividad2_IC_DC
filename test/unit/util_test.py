...
class TestUtil(unittest.TestCase):
    ...
    def test_validate_permissions(self):
        self.assertEqual(True, util.validate_permissions("1+1", "user1"))
        self.assertEqual(True, util.validate_permissions("1-1", "user1"))
        self.assertEqual(False, util.validate_permissions("1-1", "user2"))
        self.assertEqual(False, util.validate_permissions("1-1", "user2"))
        
    def test_InvalidConvertToNumber(self):
        self.assertEqual(1, util.InvalidConvertToNumber("1"))
        self.assertEqual(1.5, util.InvalidConvertToNumber("1.5"))

    def test_InvalidConvertToNumber(self):
        self.assertRaises(TypeError, util.InvalidConvertToNumber, ".")
        self.assertRaises(TypeError, util.InvalidConvertToNumber, "")        
        self.assertRaises(TypeError, util.InvalidConvertToNumber, "A")
