...
server:
	docker run --rm --volume `pwd`:/opt/calc --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -d -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
run-web:
	docker run --rm --volume `pwd`/web:/usr/share/nginx/html  --volume `pwd`/web/constants.local.js:/usr/share/nginx/html/constants.js --volume `pwd`/web/nginx.conf:/etc/nginx/conf.d/default.conf -d --name calc-web -p 80:80 nginx
...
start-sonar-server:
	...
	docker run -d --rm --stop-timeout 60 --network calc-sonar --name sonarqube-server -p 9001:9000 --volume `pwd`/sonar/data:/opt/sonarqube/data --volume `pwd`/sonar/logs:/opt/sonarqube/logs sonarqube:8.3.1-community
